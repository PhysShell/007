...
/// execution contract, and a caller-held sealed launch SOURCE (authorized by its seals at
/// acquisition, NOT an ordinary exec allowance). Such a path is a procfs MAGIC SYMLINK to a held
/// object, so it must be acquired by FOLLOWING it (never `O_NOFOLLOW`) into the exact object it points
/// at, in this PID namespace. Every other path is an ordinary filesystem path, hardened `O_NOFOLLOW`.
