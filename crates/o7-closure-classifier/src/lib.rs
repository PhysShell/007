...
/// How well established a falsification is, recorded as provenance. It does NOT
/// gate whether the fact counts: by the time a fact reaches this crate it is an
/// already-established falsification, and #147 makes falsification the WIDE
/// surface — "a concrete defect claim from any surface must be checked if it is
/// verifiable, even when that surface is inadmissible as a closure verdict."
/// Inventing a verification threshold here would narrow that deliberately wide
/// surface on this crate's own authority.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
...
/// An established defect claim, with enough provenance to re-fetch it. Carries
/// no prose: the classifier decides what an established fact does, not whether
/// some text amounts to one.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FalsificationFact {
    pub source_kind: SourceKind,
    pub stable_id: String,
    /// Present only where the surface has an immutable commit binding. Absent
    /// binding does NOT disqualify a falsification — see [`Verification`].
...
pub struct Policy {
    ...
    /// Always false in V0: enumerating a required set is not claiming the set is
    /// complete.
...
pub struct Predicate {
    ...
    /// Established falsifications, kept as their own record. A headline of
    /// `FINDING` driven by one of these would otherwise be unexplainable from
    /// the predicate alone.
