...
9. **MCP bridge — RESOLVED (operator decision).** The temporary MCP bridge is **rejected permanently** (`operator-decision.md`); no retirement question remains. The replacement open question, folded into PR-4/PR-8: the exact shape of 007's **own typed delegation protocol** — the `delegation.requested` field set, the o7d authorization checks (role/budget/depth/exact-model/permission/paths/gates), and the child-run + worktree creation contract.
