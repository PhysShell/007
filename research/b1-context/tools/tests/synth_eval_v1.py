"""Synthetic fixture family for the o7.b1.evaluation/v1 evaluator (R4B).

Everything here is SYNTHETIC. No case-0002 gold observation, task id, question id,
selected id, digest or pair count is copied. The identifiers are invented
(`synthetic-*`, `t-alpha/beta/gamma`, `obs-synthetic-*`). Only the rev3 CONTRACT
STRUCTURE and the schema constants (byte_budget 20000, record_budget 32, the
canonicalization id, the arm/family constants) are reused — those are contract
shape, not case content.

`materialize(dest)` writes a coherent all-PASS input set plus a matching
rev3-structured contract with correct digests, and returns the CLI arg list. The
committed base set lives under tests/fixtures/evaluation-v1-synthetic/; tests copy
it into a tmp dir and mutate.
"""
...
def _prov():
    return [{"source_id": SRC, "source_class": "repo_record", "digest": SRC_DIGEST}]
...
# Selected sets per task (drives the N-choose-2 shapes):
#   alpha = {a,b,c,d}         beta = {c,d,e,dep-1,dep-2}     gamma = {a,b}
#   alpha vs beta   -> incomparable
#   alpha vs gamma  -> left_strict_superset (alpha superset)
#   beta  vs gamma  -> incomparable
SELECTED = {
...
QUESTIONS = {
    "t-alpha": [{"id": "qa1", "text": "?", "required_observation_ids": ["obs-synthetic-a"]},
                {"id": "qa2", "text": "?", "required_observation_ids": ["obs-synthetic-b"]}],
    "t-beta": [{"id": "qb1", "text": "?", "required_observation_ids": ["obs-synthetic-c"]}],
    "t-gamma": [{"id": "qg1", "text": "?", "required_observation_ids": ["obs-synthetic-a"]}],
}
RELATION_REQS = {
    "t-alpha": [{"question": "qa1", "from": "obs-synthetic-a", "kind": "supersedes",
    ...
    "t-beta": [{"question": "qb1", "from": "obs-synthetic-c", "kind": "depends_on",
                "direction": "outgoing", "depth": 1, "match": "all",
                "endpoint_policy": "all_current_targets_materialized",
                "gold_derived_targets": ["obs-synthetic-dep-1", "obs-synthetic-dep-2"],
...
def context(task_id):
    by = _obs_by_id()
    sel = [dict(by[oid], selection_reason="synthetic relevance",
                selection_score=1.0) for oid in SELECTED[task_id]]
...
def fixture_manifest():
    return {"schema": "o7.b1.fixture-manifest/v0", "fixture_id": FIXTURE_ID,
            "negative_control": [], "raw_sources": [{"id": SRC, "digest": SRC_DIGEST}],
            "status": "synthetic"}


def derived_body_text():
    # Small, valid JSONL: 3 records, UTF-8, single trailing LF.
    recs = [{"event": i, "session": SESSION} for i in range(3)]
    return "".join(json.dumps(r, sort_keys=True) + "\n" for r in recs)


def derived_manifest(body_bytes):
    return {"schema": "o7.b1.derived-manifest/v0", "fixture_id": FIXTURE_ID,
            "sessions": [{"session_id": SESSION, "derived_byte_length": len(body_bytes),
                          "derived_digest": _sha(body_bytes), "record_count": 3,
                          "extractor_id": "o7-synthetic-extractor-v0", "extractor_version": "0",
                          "extractor_impl_digest": EXTRACTOR_DIGEST,
                          "raw_source": SRC, "raw_source_digest": SRC_DIGEST}]}
...
def materialize(dest):
    """Write the full synthetic input set + contract into `dest`. Returns dict
    with `contract` path and `input_args` (list of role:logical=path)."""
    os.makedirs(dest, exist_ok=True)
    p = lambda name: os.path.join(dest, name)

    # registry first (report/comparison reference its digest)
    reg = registry()
    reg_bytes = _write_yaml(p("tasks.yaml"), reg)
    reg_digest = _sha(reg_bytes)

    files = {}  # logical_id -> (obj, bytes)
    def wj(name, obj):
        files[name] = (obj, _write_json(p(name), obj)); return files[name][1]
    def wy(name, obj):
        files[name] = (obj, _write_yaml(p(name), obj)); return files[name][1]
    ...
    body_bytes = derived_body_text().encode("utf-8")
    with open(p("derived-body.jsonl"), "wb") as fh:
        fh.write(body_bytes)
    wj("derived-manifest.json", derived_manifest(body_bytes))
    ...
    bound_inputs = {
        ...
        "task_contexts": {t: dict(visibility="public", **{"artifact_bytes_sha256": _sha(files["context-%s.json" % t][1]),
                                                          "canonical_object_sha256": _sha(canonical_bytes(files["context-%s.json" % t][0])),
                                                          "byte_length": len(files["context-%s.json" % t][1])}) for t in TASK_FILES},
        "derived_bodies_private": [{"session_id": SESSION, "visibility": "private",
                                    "artifact_bytes_sha256": _sha(body_bytes),
                                    "byte_length": len(body_bytes), "record_count": 3}],
    }
    ...
    contract = {
        ...
        "contract_revision": 3,
        "semantic_revision_from_r2": False,
        "validation_revision": True,
        "supersedes_contract_revision": 2,
        "supersedes": {"instance": "evaluation-contract-v1-r2.yaml",
                       "artifact_bytes_sha256": "sha256:" + "3" * 64,
        ...
        "input_closure": {"required_public_structured": sorted(TASK_FILES.values()),
                          "required_private": [SESSION], "negative_control_body": "none",
    ...
    input_args = [
        ...
        "derived_body:%s=%s" % (SESSION, p("derived-body.jsonl")),
    ]
    for t in ("t-alpha", "t-beta", "t-gamma"):
