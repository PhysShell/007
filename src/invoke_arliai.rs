...
//! any error detail (ureq's error types carry no request headers). The
//! `o7` binary initializes no logger, so ureq's `log`-facade records —
//! wire-level TRACE included — go nowhere by construction.
...
pub(crate) fn call(
    ...
    call_url(ENDPOINT, prompt, schema_for_api, model, api_key, timeout)
}
...
/// `name` wrapper; `tool_choice: "none"`; `stream: false`. Split out so
...
mod tests {
    ...
    // through `call`, which pins ENDPOINT. Tests that cannot even bind a
    // loopback socket return early (environmental, mirroring the
    // validator-build precedent in invoke.rs tests).

    /// Serve exactly one connection: capture the full request (headers +
    /// `Content-Length` body), answer with `response`, close.
    fn serve_once(response: String) -> Option<(String, std::thread::JoinHandle<String>)> {
        ...
        Some((format!("http://{addr}/v1/chat/completions"), handle))
    }
    ...
    #[test]
    fn http_200_returns_raw_body_and_sends_key_and_shape() {
        let body =
            r#"{"choices": [{"message": {"role": "assistant", "content": "{\"ok\": true}"}}]}"#;
        let Some((url, handle)) = serve_once(http_response(
            "200 OK",
    ...
    #[test]
    fn http_302_is_returned_not_followed() {
        ...
        let Some((url, _handle)) = serve_once(http_response(
            "302 Found",
        ...
        let outcome = call_url(&url, "p", &tiny_schema(), "m", "k", Duration::from_secs(5));
    ...
    /// Live smoke against the real endpoint. Ignored by default: needs
    /// egress plus a real `ARLIAI_API_KEY` (and optionally
    /// `ARLIAI_SMOKE_MODEL`). Run explicitly:
    /// `ARLIAI_API_KEY=... cargo test --lib live_smoke_arliai -- --ignored`
    #[test]
    #[ignore = "live network + ARLIAI_API_KEY required"]
    fn live_smoke_arliai() {
        let key = std::env::var("ARLIAI_API_KEY").unwrap_or_default();
        assert!(!key.is_empty(), "live smoke needs ARLIAI_API_KEY set");
        let model = std::env::var("ARLIAI_SMOKE_MODEL")
            .unwrap_or_else(|_| "Mistral-Nemo-12B-Instruct-2407".to_string());
        ...
        let (status, body) = match outcome {
            ArliOutcome::Http { status, body } => (status, body),
            ArliOutcome::TimedOut => (0, b"timed out".to_vec()),
        ...
        assert_eq!(
            status,
            200,
            "live call did not return 200; evidence: {}",
            String::from_utf8_lossy(&body)
        );
        let extracted = extract_content(&body);
        assert!(
            extracted.is_ok(),
            "live 200 body did not yield JSON content ({:?}); body: {}",
            extracted,
            String::from_utf8_lossy(&body)
        );
    }
