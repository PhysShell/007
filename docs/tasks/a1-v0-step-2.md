...
S2 incorporated        ae720c15f98dcbac5e2edc3014b72e42362288f5   (PR #135)

предыдущая authority (реализовывать по ней больше нельзя):
  B1                   3b26849cc39a3391aaed46cca56be3b6715afabb   (PR #126, S1)
```

`B2` — это `B1` плюс S2: FD-1.3 требует уникальности имён членов в каждом объекте
A1 JSON, дубликат отклоняется на разборе, first-wins/last-wins запрещены. Правило
уже выполняется этой веткой (`scan.rs` отклоняет затенённый `null`, типизированная
схема — повторное поле), но выполнялось **до** того, как стало authority; смысл
этого перепривязывания в том, что теперь оно исполняется по контракту, а не по
собственной инициативе реализации.
