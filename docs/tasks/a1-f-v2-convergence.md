...
| `FD-1.4` | 200 | Per-object bounds (protocol hard maxima) | **PARTIAL** | E-1 obliges a finite protocol-hard envelope maximum; the VALUE is still owed | none yet | Envelope v2 **E-1**. Not discharged: bounded field values do NOT bound stored document bytes (FD-1.3 leaves whitespace/key order free and never rewrites content), so the maximum is derivable only under an encoding profile v2 has not chosen. S1 separately resolved this block's manifest double-bound. |
| `FD-1.5` | 219 | Aggregate bounds (evidence closure) | | | | |
| `FD-1.6` | 341 | Unknown fields and versions | | | | |
| `FD-1.7` | 364 | Media types | | | | |
| `FD-1.8` | 370 | Artifact refs, and what they identify | **PARTIAL** | size half consumed: the summing rule survives verbatim and E-1 supplies its missing maximum | none | Envelope v2 **E-1**. Splitting `size` back into halves was rejected: R5.2 added the summing rule precisely because a half-sized ref undercharges what the resolver reads. The DIGEST/identity half of this FD stays OPEN for the Envelope-v2 digest decision. |
