...
>
> **Статус: RA-0D ПРИНЯТ как design checkpoint (ревью оператора,
> 2026-08-05); блокирующих замечаний нет.** Типы не landed, RED-тесты не
> landed, контракт НЕ заморожен. Freeze наступает только когда RA-0T положит
> типы в протокольный слой и красную матрицу в тестовый — «замороженный»
> контракт без RED-формализации размораживается через пятнадцать минут после
> первого компилятора. RA-0T поставлен В ОЧЕРЕДЬ после SB-B и первого MG-C
> vertical — см. «Пререквизиты и порядок».
...
**Операторское решение о постановке (2026-08-05, ревью RA-0D).** RA-0T
логически не зависит от SB-A1, но ставится в очередь ПОСЛЕ зафиксированного
executable path проекта:

```text
A1 contract freeze (issue #95)
→ SB-A0 → SB-A1 GREEN → SB-A2 → SB-B
→ MG-C first end-to-end vertical
→ RA-0T
```

Причина не бюрократическая: RA-0T должен выбрать механизм multi-object
binding, а SB-B меняет ТОТ ЖЕ шов — manifest в `LaunchRequest`, включение
его в launch binding, передача нескольких FD через `SCM_RIGHTS`, protocol
v2, второй authorization barrier, одноразовое потребление grants. RA-0T,
начатый до SB-B, спроектирует параллельный transport: два manifest, две
хореографии, три определения «одноразового допуска». MG-C — первый реальный
потребитель SB-B, он обстреляет transport/state-machine контракт; RA-0T
после него использует проверенный механизм, а не бумажную спецификацию.

Нормативное следствие: **RA-0T не проектирует собственный транспорт
sealed-объектов — он потребляет механизм SB-B** (manifest, FD-passing,
one-shot grant) и лишь расширяет его ролями
`interpreter`/`program`/`auxiliary` и admission-типами. Список кандидатов в
§ «Multi-object binding» с этого момента читается как требования к выбору
ВНУТРИ механизма SB-B, не как приглашение построить альтернативный.

**Исключение.** RA-0T допустимо поднять сразу после SB-B, до MG-C, только
при появлении конкретного ближайшего consumer'а remote execution — запуск
скачанного скрипта, remote tool artifact, interpreter + payload admission —
с acceptance case. Не «когда-нибудь пригодится»: без реального потребителя
протокольные типы — speculative architecture, технический долг в
лабораторном халате.

**Процедура возобновления.** Ветка `claude/tirith-execution-analysis-8rf3im`
остаётся checkpoint'ом (в main не вливается на этом этапе). При старте
RA-0T: свежая ветка от актуального main; перенос коммитов checkpoint'а;
пересверка evidence anchors с тогдашними ревизиями (`request.rs`,
`sealed.rs`, `sandboy-boundary.md`, pinned Tirith); выбор ОДНОГО
multi-object механизма; и только затем типы и RED-матрица.
