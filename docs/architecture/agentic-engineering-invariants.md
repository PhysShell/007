...
| Logging | **enforced with constraints** | structured run/event artifacts | Logs must be sufficient for replay and diagnosis, but credentials, environment dumps and unbounded raw model output are forbidden. "Log everything" is not a safe policy. |
