...
## Recovery (on open)
1. Apply versioned migrations (idempotent).
2. Run `PRAGMA quick_check` (the cheaper, justified variant of `integrity_check`); a
   non-`ok` result fails closed.
3. `recover_scan()` reports runs/attempts still in `running` state — i.e. interrupted
   by whatever stopped the previous process.
4. **Opening never changes a status.** Marking those as `interrupted` is an explicit,
