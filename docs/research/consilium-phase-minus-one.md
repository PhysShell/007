...
**Recommendation:** do **not** depend on Consilium as a library and do **not** fork it wholesale. **Vendor a chosen set of low-coupling, test-proven modules** (verify oracle, process lifecycle, worktree isolation + trust store, quota/ledger storage shape, event/protocol/React UI, MCP schemas) and **adapt them so o7d owns the verdict, the ledger, and model policy**; stand up Consilium's MCP attached-conductor as a **temporary sandboxed advisory bridge** to unblock delegation now; and **reject** the silent-failover core, the post-hoc transcript, the conductor's verdict ownership, and the in-process Tauri shell (use them only as reference + fixtures).
...
13. **Can its MCP be the delegation protocol?** As a **schema template + temporary bridge**, not as the source of truth.
14. **Recommended integration sequence:** ledger+lifecycle → worktree+verify+trust (o7d-owned) → Exact policy + drift kill-switch → cockpit → retire the MCP bridge. See §13.
...
- **adapt:** Adapter trait, Claude & Codex adapters, AgentEvent, failure classification, **session runner** (needs process-group reap + Sandboy + async cancel), **verifier** (needs decision relocated to o7d + trusted command source + Sandboy + fail-closed), council, conduct (driver only), review parser, quota store, config, auth/doctor, worktree safety (extract), trusted-verify commands (wire), MCP (bridge+schema), Axum server (scaffolding), wire protocol, **React transport** (useSession / protocol client / run creation).
- **reference:** topology.
- **reject:** resilience/failover, model-health auto-upgrade, transcript store, Tauri shell.

> Reclassification note (per independent-gate review): `session runner` and `verifier` were moved **adopt → adapt** because reuse is not "largely as-is" (both need Sandboy + o7d ownership changes), and **React UI was split** into an *adopt* presentational/reducer half and an *adapt* transport half (useSession/protocol client/run-creation), so acquisition effort is not understated.

## 10. Recommended strategy
**Hybrid: vendor selected modules (B) + temporary sandboxed MCP bridge (C), reject the incompatible core (D).** Exact module boundaries in [`strategy-comparison.md`](../../evidence/phase-minus-one/strategy-comparison.md). Vendor `verify.rs`, `sessions/runner`, `quota.rs`, `safety/git+fs+trust`, `event/protocol`+React UI, MCP schemas, and the `eval.rs` oracle pattern — adapting each so o7d owns verdict/ledger/policy. Run Consilium's MCP server inside Sandboy as an advisory stopgap. Reject `resilience.rs`/`models.rs`/`transcript.rs`/conduct's verdict/Tauri (reference + fixtures only).
...
1. **Append-only event ledger** (extend the `quota.rs` SQLite pattern): run-event rows, per-run monotonic `seq`, `?since=` cursor, fsync + recovery reader. Stop-gate: crash mid-run loses no committed event.
2. **Worker lifecycle + Sandboy** (vendor `sessions/runner`): process-group reap; run inside Sandboy. Stop-gate: cancel kills the whole group; a worker only runs inside a Sandboy boundary.
3. **Worktree-isolation crate** (extract `safety/git+fs`) + **wire the trust store** (`safety/trust+commands`) into a vendored **`verify.rs`** whose *decision* is lifted to o7d. Stop-gate: o7d is sole worktree owner; no untrusted repo command runs without trust-on-first-use; verify runs only in Sandboy; not-run verify blocks acceptance.
4. **Extended events + ModelPolicy::Exact + drift kill-switch**: add model-info/model-drift, permission.requested, rate_limit, tool.started/completed to `event.rs`; enforce a single-rung Exact policy at the (rejected) failover choke point; halt on normalized model-id mismatch. Stop-gate: any observed model change under Exact halts the run.
5. **First vertical slice (answer #15):** one Claude worker → Sandboy + detached worktree → o7d-owned verdict via the verify gate → append-only ledger → Exact drift kill-switch. End-to-end single delegated subtask.
6. **Cockpit** (vendor `ui/src`+`protocol.rs`): rewrite `handle_session` for a run registry + ledger cursor; UI never launches a run; socket close does not kill it.
7. **Temporary MCP bridge** (Consilium MCP in Sandboy, advisory) — with a defined retirement trigger once vendored workers cover its capabilities.
